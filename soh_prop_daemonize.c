/*
* soh_prop_daemonize.c
* daemon for SOH propshot page
...
static void skeleton_daemon()
{
    ...
    */
    ...
    openlog ("./log_sh_prop", LOG_PID, LOG_DAEMON); 
    */
...
int main()
{
    ...
    while(1)
    {
        /* run the shell script for propshot. this checks whther previous */
        /* prcoess is still running before starting a new process.        */

        fp = popen("/data/mta4/Script/SOH/soh_prop_script.sh", "r"); 
        /* fp = popen("/data/mta4/Script/SOH/copy_data_from_occ_part.py prop", "r");  */

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta4/Script/SOH/Logs/soh_prop_log", "2");
            fprintf(fo, "Failed to run prop SOH process\n");
            fclose(fo);
            break;
        }else{
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta4/Script/SOH/Logs/soh_prop_log", "a");
    ...
    fo = fopen("./zspace", "w");
    fprintf(fo, "SOH propshot daemon stopped. Restart the process:\n\n");
    fprintf(fo, "\t\tnohup /data/mta4/Script/SOH/soh_prop_daemonize &\n\n");
