/*
* soh_sc_config_daemonize.c
* daemon for SOH sc_configshot page
...
static void skeleton_daemon()
{
    ...
    */
    ...
    openlog ("./log_sh_sc_config", LOG_PID, LOG_DAEMON); 
    */
...
int main()
{
    ...
    while(1)
    {
        /* run the shell script for sc_configshot. this checks whther previous */
        /* prcoess is still running before starting a new process.        */

        fp = popen("/data/mta4/Script/SOH/soh_sc_config_script.sh", "r"); 
        /* fp = popen("/data/mta4/Script/SOH/copy_data_from_occ_part.py sc_config", "r");  */

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta4/Script/SOH/Logs/soh_sc_config_log", "2");
            fprintf(fo, "Failed to run sc_config SOH process\n");
            fclose(fo);
            break;
        }else{
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta4/Script/SOH/Logs/soh_sc_config_log", "a");
    ...
    fo = fopen("./zspace", "w");
    fprintf(fo, "SOH sc_configshot daemon stopped. Restart the process:\n\n");
    fprintf(fo, "\t\tnohup /data/mta4/Script/SOH/soh_sc_config_daemonize &\n\n");
