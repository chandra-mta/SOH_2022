...
#
    try:
        out   = maude.get_msids('AOGBIAS1', start, stop, channel='ASVT')
...
#
        ...
        chk   =  extract_blob_data(msid_list, mdict, start, stop, part)
#
#--- if it is still "empty" try the last 24 hrs
#
        if chk == 'stop':
            mk = 1
            start = stop - mk*86400
            while chk == 'stop':
                ...
                if chk == 'stop':
                    mk += 1
#
#--- if it needs to check more than 5 days ago, send email to admin
#
                    if mk > 5:
                        send_warning_email()
                        write_run_file('0', part)
...
#
    try:
        ...
        out   = maude.get_msids('CCSDSVCD', start, stop, channel='ASVT')
        val   = str((list(out['data'][0]['values']))[-1])
        if val == 'NaN':
            return 'stop'
#
...
#
        for nk in range(0, len(msid_short)):
            ...
            try:
                ...
                try:
#
#--- some values are 999 or 0. check whether noe 999 or 0 value exist in the data array
#
                    fval = float(val)
...
def get_u_p():
    ...
    data = read_file_data('/home/mta/.netrc')
