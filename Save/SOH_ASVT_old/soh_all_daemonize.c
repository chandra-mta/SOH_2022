...
int main()
{
    ...
    while(1)
    {
        ...
        fp = popen("/data/mta/Script/SOH_ASVT/soh_all_script.sh", "r");

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta/Script/SOH_ASVT/Logs/soh_all_log", "2");
            fprintf(fo, "Failed to run all SOH_ASVT process\n");
            fclose(fo);
            break;
        }else{
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta/Script/SOH_ASVT/Logs/soh_all_log", "a");
    ...
    fo = fopen("./zspace", "w");
    fprintf(fo, "SOH_ASVT all page daemon stopped. Restart the process:\n\n");
    fprintf(fo, "\t\tnohup /data/mta/Script/SOH_ASVT/soh_all_daemonize &\n\n");
    fprintf(fo, "This process should be run on han-v as mta.\n");
    fclose(fo);

    system("cat ./zspace |mailx -s 'SOH_ASVT all page daemon stopped' tisobe@cfa.harvard.edu");
    system("rm -rf ./zsapce");
