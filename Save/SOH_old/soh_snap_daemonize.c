...
int main()
{
    ...
    while(1)
    {
        ...
        /* fp = popen("/data/mta/Script/SOH/soh_snap_script.sh", "r"); */
        fp = popen("/data/mta/Script/SOH/copy_data_from_occ_part.py snap", "r"); 

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta/Script/SOH/Logs/soh_snap_log", "2");
            fprintf(fo, "Failed to run snap SOH process\n");
            fclose(fo);
            break;
        }else{
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta/Script/SOH/Logs/soh_snap_log", "a");
    ...
    fo = fopen("./zspace", "w");
    fprintf(fo, "SOH snapshot daemon stopped. Restart the process:\n\n");
    fprintf(fo, "\t\tnohup /data/mta/Script/SOH/soh_snap_daemonize &\n\n");
