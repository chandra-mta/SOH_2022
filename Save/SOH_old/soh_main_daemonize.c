...
int main()
{
    ...
    while(1)
    {
        ...
        fp = popen("/data/mta/Script/SOH/soh_main_script.sh", "r");

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta/Script/SOH/Logs/soh_main_log", "2");
            fprintf(fo, "Failed to run main SOH process\n");
            fclose(fo);
            break;
        }else{
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta/Script/SOH/Logs/soh_main_log", "a");
    ...
    fo = fopen("./zspace", "w");
    fprintf(fo, "SOH main page daemon stopped. Restart the process:\n\n");
    fprintf(fo, "\t\tnohup /data/mta/Script/SOH/soh_main_daemonize &\n\n");
    fprintf(fo, "This process should be run on han-v as mta.\n");
    fclose(fo);

    system("cat ./zspace |mailx -s 'SOH main page daemon stopped' tisobe@cfa.harvard.edu");
    system("rm -rf ./zsapce");
