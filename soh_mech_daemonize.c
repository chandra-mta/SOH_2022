/*
* soh_mech_daemonize.c
* daemon for SOH mechshot page
...
static void skeleton_daemon()
{
    ...
    */
    ...
    openlog ("./log_sh_mech", LOG_PID, LOG_DAEMON); 
    */
...
int main()
{
    ...
    while(1)
    {
        /* run the shell script for mechshot. this checks whther previous */
        /* prcoess is still running before starting a new process.        */

        fp = popen("/data/mta4/Script/SOH/soh_mech_script.sh", "r"); 
        /* fp = popen("/data/mta4/Script/SOH/copy_data_from_occ_part.py mech", "r");  */

        /* check the script runs correctly */

        if (fp == NULL){
            fo  = fopen("/data/mta4/Script/SOH/Logs/soh_mech_log", "2");
            fprintf(fo, "Failed to run mech SOH process\n");
            fclose(fo);
            break;
        }else{
        ...
        /* update the log file --- this is used to see whether the process is still running */
            ...
            fo  = fopen("/data/mta4/Script/SOH/Logs/soh_mech_log", "a");
    ...
    fo = fopen("./zspace", "w");
    fprintf(fo, "SOH mechshot daemon stopped. Restart the process:\n\n");
    fprintf(fo, "\t\tnohup /data/mta4/Script/SOH/soh_mech_daemonize &\n\n");
